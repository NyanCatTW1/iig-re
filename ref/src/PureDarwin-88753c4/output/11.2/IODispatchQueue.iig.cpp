...
IODispatchQueue::_Dispatch(IODispatchQueue * self, const IORPC rpc)
{
    ...
    switch (msg->msgid)
    {
        ...
        {
            union {
                kern_return_t (IODispatchQueue::*fIn)(IODispatchQueue_SetPort_Args);
                IODispatchQueue::SetPort_Handler handler;
            } map;
            map.fIn = &IODispatchQueue::SetPort_Impl;
            ret = IODispatchQueue::SetPort_Invoke(rpc, self, map.handler);
            break;
        }
#endif /* !KERNEL */
