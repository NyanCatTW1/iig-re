/* iig(DriverKit-256.100.5 Mar 10 2023 21:35:11) generated from OSObject.iig */
        ...
        SetDispatchQueue_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(OSObject_SetDispatchQueue_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(OSObject_SetDispatchQueue_Rpl)) return (kIOReturnIPCError);
        ...
        CopyDispatchQueue_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(OSObject_CopyDispatchQueue_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(OSObject_CopyDispatchQueue_Rpl)) return (kIOReturnIPCError);
    if (strnlen(&rpc.message->content.__name[0], sizeof(rpc.message->content.__name)) >= sizeof(rpc.message->content.__name)) return kIOReturnBadArgument;
