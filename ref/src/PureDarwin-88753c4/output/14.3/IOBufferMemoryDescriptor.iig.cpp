/* iig(DriverKit-256.100.5 Mar 10 2023 21:35:11) generated from IOBufferMemoryDescriptor.iig */
        ...
        Create_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOBufferMemoryDescriptor_Create_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOBufferMemoryDescriptor_Create_Rpl)) return (kIOReturnIPCError);
        ...
        SetLength_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOBufferMemoryDescriptor_SetLength_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOBufferMemoryDescriptor_SetLength_Rpl)) return (kIOReturnIPCError);
