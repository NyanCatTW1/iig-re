/* iig(DriverKit-73.0.1) generated from IODispatchSource.iig */
...
IODispatchSource::_Dispatch(IODispatchSource * self, const IORPC rpc)
{
    ...
    switch (msg->msgid)
    {
        ...
        {
            union {
                kern_return_t (IODispatchSource::*fIn)(IODispatchSource_SetEnable_Args);
                IODispatchSource::SetEnable_Handler handler;
            } map;
            map.fIn = &IODispatchSource::SetEnable_Impl;
            ret = IODispatchSource::SetEnable_Invoke(rpc, self, map.handler);
            break;
        }
