...
IOUserClient::_Dispatch(IOUserClient * self, const IORPC rpc)
{
    ...
    switch (msg->msgid)
    {
        ...
        {
            union {
                kern_return_t (IOUserClient::*fIn)(IOUserClient_CreateMemoryDescriptorFromClient_Args);
                IOUserClient::CreateMemoryDescriptorFromClient_Handler handler;
            } map;
            map.fIn = &IOUserClient::CreateMemoryDescriptorFromClient_Impl;
            ret = IOUserClient::CreateMemoryDescriptorFromClient_Invoke(rpc, self, map.handler);
            break;
        }
#endif /* !KERNEL */
        ...
        _ExternalMethod_Handler func)
{
...

    {
        ...
        {
            rpc.reply->content.__hdr.objectRefs = 0;
            rpc.reply->content.scalarOutputCount = 0;
            rpc.reply->structureOutput__descriptor.type    = MACH_MSG_OOL_DESCRIPTOR;
            rpc.reply->structureOutput__descriptor.copy    = MACH_MSG_VIRTUAL_COPY;
            rpc.reply->structureOutput__descriptor.address = NULL;
            rpc.reply->structureOutput__descriptor.size    = 0;
        }
    }
    return (kIOReturnSuccess);
