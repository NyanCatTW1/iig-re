/* iig(DriverKit-427.100.5 Mar 12 2025 20:11:39) generated from OSObject.iig */
...
struct OSObject_SetDispatchQueue_Msg_With_Content
{
    IORPCMessageMach           mach;
    mach_msg_port_descriptor_t __object__descriptor;
    mach_msg_port_descriptor_t queue__descriptor;
...
struct OSObject_SetDispatchQueue_Rpl_With_Content
{
    IORPCMessageMach           mach;
...
#else /* KERNEL */
        const struct OSObject_SetDispatchQueue_Msg_With_Content * message;
#endif /* KERNEL */
        struct OSObject_SetDispatchQueue_Rpl_With_Content       * reply;
        uint32_t sendSize;
        uint32_t replySize;
#ifdef KERNEL
        const struct OSObject_SetDispatchQueue_Msg_Content * kernelContent;
#endif /* KERNEL */
    };
...
struct OSObject_CopyDispatchQueue_Msg_With_Content
{
    IORPCMessageMach           mach;
    mach_msg_port_descriptor_t __object__descriptor;
...
struct OSObject_CopyDispatchQueue_Rpl_With_Content
{
    IORPCMessageMach           mach;
    mach_msg_port_descriptor_t queue__descriptor;
...
#else /* KERNEL */
        const struct OSObject_CopyDispatchQueue_Msg_With_Content * message;
#endif /* KERNEL */
        struct OSObject_CopyDispatchQueue_Rpl_With_Content       * reply;
        uint32_t sendSize;
        uint32_t replySize;
#ifdef KERNEL
        const struct OSObject_CopyDispatchQueue_Msg_Content * kernelContent;
#endif /* KERNEL */
    };
        ...
        OSDispatchMethod supermethod)
{
    ...
    union
    {
        OSObject_SetDispatchQueue_Msg_With_Content msg;
        struct
        {
            OSObject_SetDispatchQueue_Rpl_With_Content rpl;
            mach_msg_max_trailer_t trailer;
        } rpl;
    } buf;
    struct OSObject_SetDispatchQueue_Msg_With_Content * msg = &buf.msg;
    struct OSObject_SetDispatchQueue_Rpl_With_Content * rpl = &buf.rpl.rpl;
        ...
        OSDispatchMethod supermethod)
{
    ...
    union
    {
        OSObject_CopyDispatchQueue_Msg_With_Content msg;
        struct
        {
            OSObject_CopyDispatchQueue_Rpl_With_Content rpl;
            mach_msg_max_trailer_t trailer;
        } rpl;
    } buf;
    struct OSObject_CopyDispatchQueue_Msg_With_Content * msg = &buf.msg;
    struct OSObject_CopyDispatchQueue_Rpl_With_Content * rpl = &buf.rpl.rpl;
        ...
        SetDispatchQueue_Handler func)
{
    ...
    OSObject_SetDispatchQueue_Rpl_With_Content * reply = rpc.reply;
#ifdef KERNEL
    const OSObject_SetDispatchQueue_Msg *         message = rpc.message;
    const OSObject_SetDispatchQueue_Msg_Content * messageContent = rpc.kernelContent;
#else /* KERNEL */
    const OSObject_SetDispatchQueue_Msg_With_Content * message = rpc.message;
#endif /* KERNEL */
    ...
    if (OSObject_SetDispatchQueue_Msg_ObjRefs != MESSAGE_CONTENT(__hdr.objectRefs)) return (kIOReturnIPCError);
    if (rpc.sendSize < sizeof(OSObject_SetDispatchQueue_Msg_With_Content)) return (kIOReturnIPCError);
    ...
    reply->content.__hdr.msgid = OSObject_SetDispatchQueue_ID;
    ...
    reply->content.__hdr.objectRefs = OSObject_SetDispatchQueue_Rpl_ObjRefs;
        ...
        CopyDispatchQueue_Handler func)
{
    ...
    OSObject_CopyDispatchQueue_Rpl_With_Content * reply = rpc.reply;
#ifdef KERNEL
    const OSObject_CopyDispatchQueue_Msg *         message = rpc.message;
    const OSObject_CopyDispatchQueue_Msg_Content * messageContent = rpc.kernelContent;
#else /* KERNEL */
    const OSObject_CopyDispatchQueue_Msg_With_Content * message = rpc.message;
#endif /* KERNEL */
#if __has_builtin(__builtin_assume)
    __builtin_assume(reply != NULL);
#endif /* __has_builtin(__builtin_assume) */
    ...
    if (OSObject_CopyDispatchQueue_Msg_ObjRefs != MESSAGE_CONTENT(__hdr.objectRefs)) return (kIOReturnIPCError);
    if (rpc.sendSize < sizeof(OSObject_CopyDispatchQueue_Msg_With_Content)) return (kIOReturnIPCError);
    if (reply != NULL && rpc.replySize < sizeof(*reply)) return (kIOReturnIPCError);
    if (strnlen(&MESSAGE_CONTENT(__name[0]), sizeof(MESSAGE_CONTENT(__name))) >= sizeof(MESSAGE_CONTENT(__name))) return kIOReturnBadArgument;
    ...
    reply->content.__hdr.msgid = OSObject_CopyDispatchQueue_ID;
    reply->content.__hdr.flags = kIORPCMessageOneway;
    reply->mach.msgh.msgh_id   = kIORPCVersion190615Reply;
    reply->mach.msgh.msgh_size = sizeof(*reply);
    reply->mach.msgh_body.msgh_descriptor_count = 1;
    reply->content.__hdr.objectRefs = OSObject_CopyDispatchQueue_Rpl_ObjRefs;
