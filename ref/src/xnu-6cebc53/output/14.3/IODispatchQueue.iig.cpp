/* iig(DriverKit-256.100.5 Mar 10 2023 21:35:11) generated from IODispatchQueue.iig */
        ...
        SetPort_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IODispatchQueue_SetPort_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IODispatchQueue_SetPort_Rpl)) return (kIOReturnIPCError);
        ...
        Create_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IODispatchQueue_Create_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IODispatchQueue_Create_Rpl)) return (kIOReturnIPCError);
