/* iig(DriverKit-256.100.5 Mar 10 2023 21:35:11) generated from OSAction.iig */
        ...
        Create_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(OSAction_Create_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(OSAction_Create_Rpl)) return (kIOReturnIPCError);
        ...
        CreateWithTypeName_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(OSAction_CreateWithTypeName_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(OSAction_CreateWithTypeName_Rpl)) return (kIOReturnIPCError);
    if (((OSObject *) rpc.message->content.typeName) != NULL && OSDynamicCast(OSString, (OSObject *) rpc.message->content.typeName) == NULL) { return kIOReturnBadArgument; } 
    target = OSDynamicCast(OSObject, (OSObject *) rpc.message->content.target);
    if (!target && rpc.message->content.target) return (kIOReturnBadArgument);
        ...
        Aborted_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(OSAction_Aborted_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(OSAction_Aborted_Rpl)) return (kIOReturnIPCError);
