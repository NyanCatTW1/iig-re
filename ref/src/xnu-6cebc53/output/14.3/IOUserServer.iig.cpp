/* iig(DriverKit-256.100.5 Mar 10 2023 21:35:11) generated from IOUserServer.iig */
        ...
        Create_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOUserServer_Create_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOUserServer_Create_Rpl)) return (kIOReturnIPCError);
    if (((OSObject *) rpc.message->content.bundleID) != NULL && OSDynamicCast(OSString, (OSObject *) rpc.message->content.bundleID) == NULL) { return kIOReturnBadArgument; } 
        ...
        Exit_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOUserServer_Exit_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOUserServer_Exit_Rpl)) return (kIOReturnIPCError);
        ...
        LoadModule_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOUserServer_LoadModule_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOUserServer_LoadModule_Rpl)) return (kIOReturnIPCError);
