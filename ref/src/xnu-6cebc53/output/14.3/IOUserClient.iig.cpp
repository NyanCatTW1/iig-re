/* iig(DriverKit-256.100.5 Mar 10 2023 21:35:11) generated from IOUserClient.iig */
        ...
        const OSMetaClass * targetActionClass)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOUserClient_AsyncCompletion_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOUserClient_AsyncCompletion_Rpl)) return (kIOReturnIPCError);
        ...
        CopyClientMemoryForType_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOUserClient_CopyClientMemoryForType_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOUserClient_CopyClientMemoryForType_Rpl)) return (kIOReturnIPCError);
        ...
        CreateMemoryDescriptorFromClient_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOUserClient_CreateMemoryDescriptorFromClient_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOUserClient_CreateMemoryDescriptorFromClient_Rpl)) return (kIOReturnIPCError);
        ...
        CopyClientEntitlements_Handler func)
{
    ...
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOUserClient_CopyClientEntitlements_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOUserClient_CopyClientEntitlements_Rpl)) return (kIOReturnIPCError);

    ret = (*func)(target,
        &rpc.reply->content.entitlements);
    ...
    rpc.reply->entitlements__descriptor.type = MACH_MSG_OOL_DESCRIPTOR;
    rpc.reply->entitlements__descriptor.copy = MACH_MSG_VIRTUAL_COPY;
    rpc.reply->entitlements__descriptor.address = (void *) __builtin_offsetof(IOUserClient_CopyClientEntitlements_Rpl_Content, entitlements);
    rpc.reply->entitlements__descriptor.size = 0;
        ...
        _ExternalMethod_Handler func)
{
    ...
    if (5 != rpc.message->mach.msgh_body.msgh_descriptor_count) return (kIOReturnIPCError);
    if (IOUserClient__ExternalMethod_Msg_ObjRefs != rpc.message->content.__hdr.objectRefs) return (kIOReturnIPCError);
    if (rpc.message != NULL && rpc.sendSize < sizeof(IOUserClient__ExternalMethod_Msg)) return (kIOReturnIPCError);
    if (rpc.reply != NULL && rpc.replySize < sizeof(IOUserClient__ExternalMethod_Rpl)) return (kIOReturnIPCError);
    if (((OSObject *) rpc.message->content.structureInput) != NULL && OSDynamicCast(OSData, (OSObject *) rpc.message->content.structureInput) == NULL) { return kIOReturnBadArgument; } 
